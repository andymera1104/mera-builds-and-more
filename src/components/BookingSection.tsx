...
const timeSlots = ["Morning (8–11 AM)", "Midday (11 AM–2 PM)", "Afternoon (2–5 PM)"] as const;
  ...
  if (sent) {
    return (
      <div className="mt-12 bg-steel-2 p-8 sm:p-12 rounded-[min(1vw,16px)] outline-1 -outline-offset-1 outline-amber/30 text-center">
        <p className="font-display text-5xl text-amber leading-none">¡Agendado!</p>
        <h3 className="mt-4 font-display text-3xl text-foreground tracking-wide">
          Your visit request was booked
        </h3>
        <p className="mt-3 text-foreground/60 text-sm leading-relaxed max-w-[46ch] mx-auto">
          Recibimos tu solicitud de visita. Te llamamos para confirmar el día y la hora exacta —
          sin compromiso, la visita y la cotización son gratis.
        </p>
    ...
    <div className="mt-12 grid lg:grid-cols-5 gap-10 items-start">
      <div className="lg:col-span-2">
        ...
        <h3 className="mt-6 font-display text-4xl text-foreground tracking-wide">
          Book a Free Visit.
          <br />
          <span className="text-amber">Agenda tu visita.</span>
        </h3>
        <p className="mt-4 text-foreground/60 text-sm leading-relaxed max-w-[38ch]">
          Pick a day that works for you and we come to the project, take measurements and give you
          a written quote on the spot. Sin costo y sin compromiso.
        </p>
          ...
          try {
            ...
            setError(
              err instanceof Error
                ? err.message
                : "No pudimos agendar tu visita. Intenta de nuevo.",
            );
          } finally {
            setSending(false);
          }
      ...
      >
        ...
        <label className="sm:col-span-1 block">
          <span className={labelClass}>Preferred day / Día preferido</span>
          <input type="date" name="date" required min={today} className={fieldClass} />
        </label>
        <label className="sm:col-span-1 block">
          <span className={labelClass}>Time / Hora</span>
        ...
        <label className="sm:col-span-2 block">
          <span className={labelClass}>Project address / Dirección (opcional)</span>
          <input type="text" name="address" maxLength={200} className={fieldClass} placeholder="123 W Main St, Phoenix, AZ" />
        </label>
        <label className="sm:col-span-2 block">
          <span className={labelClass}>Notes / Notas (opcional)</span>
          <textarea
            name="notes"
            rows={2}
            maxLength={1000}
        ...
        >
          {sending ? "Booking…" : "Book My Free Visit"}
        </button>
      </form>
    </div>
  );
